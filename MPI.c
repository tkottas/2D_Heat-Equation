...
int main (int argc, char *argv[]) {
    ...
    rc,start,end,               /* misc */
    i,j,checki,checkj,ix,iy,iz,it,sum1,sum2,flag=0,allflag=1;              /* loop variables */
    MPI_Status status;
    ...
    //printf("Task %d Starting work with neighbors --> up : %d ,down : %d,right : %d ,left : %d blocks_s : %d. Beginning time steps...\n",taskid,up,down,right,left,blocks_s);
   ...
   for (it = 1; it <= STEPS; it++){
   	/*if(it%5==0){
            flag=0;
            for(checki=1;(checki<=blocks_s)&&(flag==0);checki++){
                for(checkj=1;checkj<=blocks_s;checkj++){
                    //printf ("Task %d : Checking (%f==%f) \n",taskid,localu[1][checki][checkj],localu[0][checki][checkj]);

                    if((localu[1][checki][checkj]-localu[0][checki][checkj] >0.01)||((localu[0][checki][checkj]-localu[1][checki][checkj])>0.01)){
                       // printf ( "Task %d : ( %d - %d ) = %f \n",taskid,checki,checkj,localu[1][checki][checkj]-localu[0][checki][checkj]);
                        flag=1;
                        break;
                    }
                }
            ...
            MPI_Allreduce(&flag, &allflag, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
            if(allflag==0)
                break;
...
void updateouter(int up,int down,int left,int right,int sizep, int size, int task,int nblocks, float *u1, float *u2) {
    ...
    int ix,iy,hstart,hend,vstart,vend;
